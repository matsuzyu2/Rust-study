...
ユイがエディタを開いて、意気揚々とファイル読み込みのコードを書いている。

> 👩‍💻 **ユイ**: 「よし、設定ファイルを読み込む処理を書くぞ！」
...
> 👩‍💻 **ユイ**: 「できた！動くし、これでOKですよね？」
>
> 👩‍🏫 **レイ**: （後ろから画面を見て）「ユイちゃん…そのコード、**本番で使ったら危険**よ。」
>
> 👩‍💻 **ユイ**: 「えっ！？ちゃんと動いてますよ？」
>
> 👩‍🏫 **レイ**: 「今は動いてる。でも、`config.json`が存在しなかったら？」

```
# config.json がない状態で実行
...
> 👩‍💻 **ユイ**: 「うわっ！クラッシュした！」
>
> 👩‍🏫 **レイ**: 「そう。`unwrap()`は『成功すると思い込んでる』コードなの。失敗したら**プログラム全体がクラッシュ**するわ。」
>
> 👩‍💻 **ユイ**: 「えぇ…じゃあtry-catchで囲めば…あれ、Rustってtry-catchどう書くんですか？」
>
> 👩‍🏫 **レイ**: （クスッと笑って）「ユイちゃん、Rustには**try-catchがない**のよ。」
>
> 👩‍💻 **ユイ**: 「えっっ！？例外処理ないんですか！？じゃあエラーってどう処理するんですか！？このままだと本番で事故りますよ！？」
>
> 👩‍🏫 **レイ**: 「落ち着いて。Rustは『例外を投げる』んじゃなくて、『エラーを値として返す』の。これが**めちゃくちゃ優れてる**アプローチなのよ。」
>
> 👩‍💻 **ユイ**: 「エラーを…値として…？」
>
> 👩‍🏫 **レイ**: 「うん。でもその前に、なぜ`unwrap()`が危険なのか、そしてなぜRustがこのアプローチを選んだのか、ちゃんと説明するわね。」

### unwrapの問題 — 実務での失敗談

> 👩‍🏫 **レイ**: 「実は私も、Rust始めたばかりの頃に`unwrap()`地獄にハマったことがあってね…」
>
> 👩‍💻 **ユイ**: 「先輩も？」
>
> 👩‍🏫 **レイ**: 「うん。『とりあえず動かしたい』って気持ちで、`unwrap()`を**42箇所**に書いたの。」
>
> 👩‍💻 **ユイ**: 「42箇所…！」
>
> 👩‍🏫 **レイ**: 「で、本番デプロイして3日後、深夜2時にPagerDuty（アラートサービス）が鳴ったの。DBコネクションが一時的に切れて、`unwrap()`がpanicして、サービス全停止。」
>
> 👩‍💻 **ユイ**: 「ひぃ…！深夜2時…」
>
> 👩‍🏫 **レイ**: 「そこから徹夜で全部の`unwrap()`を適切なエラーハンドリングに書き換えたわ。その教訓から言えるのは、**`unwrap()`はテストコードとプロトタイプ以外では使うな**ってこと。」
>
> 👩‍💻 **ユイ**: 「わ、分かりました…。じゃあ、どうすればいいんですか？」
>
> 👩‍🏫 **レイ**: 「まず、エラー処理の歴史から話すわね。Rustがなぜこのアプローチを選んだか分かると、コードの書き方が変わるから。」
...
> 👩‍💻 **ユイ**: 「まず、最初に書いた`unwrap()`だらけのコードがダメな理由が分かりました。ファイルがなかったり、ネットワークが切れたりしたら**プログラム全体がクラッシュ**しちゃうんですよね。」
>
> 👩‍🏫 **レイ**: 「そう。先輩の深夜2時のPagerDuty事件を忘れないでね。」
>
> 👩‍💻 **ユイ**: 「忘れません…！で、Rustにはtry-catchがないけど、代わりに**Result型**がある。これは『エラーを値として返す』アプローチで、関数のシグネチャを見れば『この関数はエラーを返すかもしれない』って分かる。」
>
> 👩‍🏫 **レイ**: 「完璧。」
>
> 👩‍💻 **ユイ**: 「そして**`?`演算子**！これがめちゃくちゃ便利で、`match`で書くと10行くらいになるエラー伝播が、たった**1文字**でできる。しかも、エラーの型変換も自動でやってくれる。」
>
> 👩‍🏫 **レイ**: 「`?`はRust書くなら毎日使うわよ。」
>
> 👩‍💻 **ユイ**: 「あと、エラー処理の歴史も面白かったです。Cの戻り値、Javaのチェック例外、Goの多値返却…それぞれに問題があって、Rustの`Result`が**集大成**になってるっていう。」
>
> 👩‍🏫 **レイ**: 「言語は過去の失敗から学んでるのよね。」
>
> 👩‍💻 **ユイ**: 「一番衝撃だったのは、`?`演算子の歴史ですね。最初は`try!`マクロだったのが、RFC 243で`?`になって…人間工学的な改善の結果、今の形になった。」
>
> 👩‍🏫 **レイ**: 「言語設計って、そういう議論の積み重ねなのよ。」
>
> 👩‍💻 **ユイ**: 「これからは`unwrap()`は**テストコードだけ**にします！」
>
> 👩‍🏫 **レイ**: 「（嬉しそうに）その意気よ！」
